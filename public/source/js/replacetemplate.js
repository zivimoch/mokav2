$(document).ready(function() {
    ...
    $('.input-variable').on('blur', function() {
        if ($(this).text() == '') {
            ...
            $(".input-" + variableName).text('{{' + $(this).attr('name') + '}}')
        }
    });
...
function addRow(id_table) {
    ...
    var row = length - 2;
    tbody.insertBefore(mainTable.rows[row].cloneNode(true), mainTable.rows[row]);
}
...
setInterval(function() {
    document.getElementById("konten").value = document.getElementById("preview").innerHTML;
...
function update() {
    textarea.innerHTML = inputs.reduce((acc, i) => acc.replace(new RegExp(`\\{\\{\\s*${i.id}\\s*\\}\\}`, 'g'), i.value), template);
}
if (inputs.length > 0)
