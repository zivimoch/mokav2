    ...
    <div id="viewAgenda">
      <div class="modal-body"  style="margin-bottom: -15px">
        <table class="table table-bottom table-sm">
          ...
          <tr class="viewAgendaLayanan">
            <td style="width: 20%"><b>Intervensi Ke </b></td>
            <td style="width: 1%"><b>:</b></td>
                ...
                success: function (response){
                  if (response.success != true) {
                    ...
                    if($(".tabelLayanan").length > 0) {
                      $('#tabelLayanan'+$("#jumlah_intervensi_ke").val()).DataTable().ajax.reload(null, false);
  ...
  if (agenda_id != 0) {
    ...
    $('#user_id_select').empty();
      $.get(`{{ env('APP_URL') }}/agenda/edit/`+agenda_id+`?petugas=`+petugas_id, function (data) {
          ...
          keyword.forEach(e => {
            $("#viewLayanan").append(e.keyword+', ');
            $("#keyword-agenda").append('<option value="'+e.id+'" selected>'+e.keyword+'</option>');
          });
