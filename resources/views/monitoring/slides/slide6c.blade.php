...
<div class="content">
  <div class="row">
    <div class="col-md-5">
      <table id="tabelChart6c" class="table table-sm table-bordered table-hover" style="cursor:pointer; width:100%"></table>  
    </div>
    <div class="col-md-7">
      <div id="chart6c" style="display: block; width: 100%; height: 100%; padding: 0px 20px"></div>
    </div>
  </div>
</div>
<script>
  ...
  var chart6c; 
  function load_data6c() {
      if ($.fn.DataTable.isDataTable('#tabelChart6c')) {
          $('#tabelChart6c').DataTable().destroy();
      }
      ...
      regis = 1;
      arsip = 0;
      klasifikasi = 'bentuk_kekerasan';

      titleChart6c = $('#titleChart6c').html(klasifikasi);
      ...
      $.ajax({
          ...
          success: function(response) {
              ...
              if (!chart6c) {
                  ...
                  chart6c = new ApexCharts(document.querySelector("#chart6c"), options);
                  chart6c.render();
              } else {
                  // Update chart data if it exists
                  chart6c.updateOptions({
                      series: jumlah,
                      labels: labels
                  });
              }
              ...
              }).buttons().container().appendTo('#tabelChart6c_wrapper .col-md-6:eq(0)');

              $('#tabelChart6c_filter').css({'float':'right','display':'inline-block; background-color:black'});
          },
