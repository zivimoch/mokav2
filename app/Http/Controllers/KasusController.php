...
class KasusController extends Controller
{
    ...
    public function show(Request $request, $uuid)
    {
       ...
       ->select(
           ...
           'c.name as kota',
           'd.name as kecamatan',
           DB::raw('(SELECT GROUP_CONCAT(" ", value) FROM difabel_type WHERE klien_id = a.id) as difabel_type'),
           DB::raw('(SELECT GROUP_CONCAT(" ", value) FROM kondisi_khusus WHERE klien_id = a.id) as kondisi_khusus'),
           DB::raw('(SELECT GROUP_CONCAT(" ", value) FROM kategori_kasus WHERE klien_id = a.id) as kategori_kasus'),
           DB::raw('(SELECT GROUP_CONCAT(" ", value) FROM difabel_type WHERE klien_id = a.id) as difabel_type'),
           DB::raw('(SELECT GROUP_CONCAT(" ", value) FROM program_pemerintah WHERE klien_id = a.id) as program_pemerintah')
       )
    ...
    public function generate_nokas()
    {
        ...
        $no_kasus = $urutan_regis.'/'.date('m').'/'.date('Y');
